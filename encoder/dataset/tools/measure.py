...
class APMeter(AverageMeter):
    ...
    def reset(self):
        ...
        # Initialize empty tensors using modern API (avoid deprecated Storage constructors)
    ...
    def value(self):
        ...
        for k in range(self.scores.size(1)):
            ...
            precision = tp.div(rg) # give more importance to gt classes found with higher confidences

            # compute average precision
            ap[k] = precision[truth.byte()].sum() / max(truth.sum(), 1)

        return float(torch.sum(100 * ap) / torch.nonzero(100 * ap).size()[0])
        # return ap
